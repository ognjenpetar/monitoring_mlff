...
class _NoVerifyAdapter(HTTPAdapter):
    ...
    context.load_default_certs(), which reads the Windows certificate
    store and raises ssl.SSLError('[ASN1] nested asn1 error') on this
    machine because of a malformed certificate in that store — even
    though verify=False makes the certs unnecessary in the first place.
    """
